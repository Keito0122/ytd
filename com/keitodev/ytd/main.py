...
DEFAULT_OUTPUT = str(Path.home() / "storage" / "downloads")
...
⚠ 注意: URLは必ずダブルクォートで囲んでください。
  ...
  クオートを忘れるとシェルが "&" をバックグラウンド実行の区切りと
  解釈し、オプションが正しく渡りません。
...
def main(argv=None):
    ...
    output_dir = resolve_value(args.output, cfg.get("output"), DEFAULTS["output"])
